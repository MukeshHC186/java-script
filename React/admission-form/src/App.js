...
const formObj = {
  ...
  Student_Details: [
    ...
    {
      label:"Result Of Previous class",
      name: "rpsresult",
...
console.log(tableData);
  function submitData(){
    if(!!form_Obj?.name && !!form_Obj?.fname && !!form_Obj?.mname && !!form_Obj?.dob && !!form_Obj?.address && !!form_Obj?.course && !!form_Obj?.mobile && !!form_Obj?.email){
    ...
    <>
      <div className="App">
        ...
        <div className="form">
          ...
          <button className="btn_1" onClick={submitData} >Submit</button>&nbsp;&nbsp;
          <button className="btn_2" onClick={()=>{setFormObj(null)}}>Reset</button>
          </div>
        </div>
        <hr></hr>
